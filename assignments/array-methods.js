...
let ticketPriceTotal = runners.reduce((acc, currentItem) => {
...
// Problem 1 === Find the average donation size of each runner. ====
let averageDonation = runners.reduce((acc, currentPerson) => {
    return acc + currentPerson.donation;
}, 0);
console.log(averageDonation / runners.length);

// Problem 2 === Create a new list with the details of only those companies who donated $150 or more. Format it for JSON and display it. ===
let highDonors = runners.filter(person => person.donation >= 150);
console.log(JSON.stringify(highDonors));

// Problem 3 === Still drunk with power, the director wants to dis-invite everyone who donated less than $50. He needs a new list of only first and last names and email addresses of those people. ===
let justCheapskates = runners.filter(allDonors => allDonors.donation < 50);
let cheapSkateEmails = justCheapskates.map((justCheapskates) => {
    return { 'first_name': justCheapskates.first_name, 'last_name': justCheapskates.last_name, 'email': justCheapskates.email };
});
console.log(cheapSkateEmails);